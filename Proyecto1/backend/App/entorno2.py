...
def wait_element(by, val,timeout=10): # Esperar a que el elemento esté presente en el DOM
    try:
        WebDriverWait(browser, timeout).until(EC.presence_of_element_located((by, val)))
        time.sleep(0.5)
        return WebDriverWait(browser, timeout).until(EC.element_to_be_clickable((by, val)))
    except TimeoutException:
        logger.error(f"El elemento con {by}='{val}' no estuvo disponible o clickeable en {timeout} segundos.")
        raise

def wait_disappear(by, val, timeout=120): # Esperar a que aparezca y desaparezca el elemento en el DOM
    try:
        WebDriverWait(browser, timeout).until_not(EC.presence_of_element_located((by, val)))
    except TimeoutException:
        logger.error("El elemento no desapareció dentro del tiempo especificado.")
        raise
